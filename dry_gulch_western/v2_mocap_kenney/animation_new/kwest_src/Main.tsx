...
const LEG = 200;
// Kenney limbs are single-segment → single:true maps shoulder→hand / hip→foot angles onto uarm/thigh.
const mc = (clip: string, t: number, scale: number, speed = 1, amp = 1) => { const m = mocapPose(clip, t, { legLen: LEG, speed, amp, single: true }); return { pose: m.pose, bob: m.hipY * scale, lean: 0 }; };
...
const HF = { smile: "n", grin: "n", smirk: "n", sleep: "head_hurt", frown: "head_hurt", wink: "head_focus", talk: "head_focus" };
const TF = { smile: "n", oh: "head_hurt", laugh: "head_focus", sad: "head_hurt", happy: "head_focus", neutral: "n" };
...
export const SCENES: Sc[] = [
  ...
  { id: "sc2", start: 15.0, end: 21.3, bg: "street", solve: (t) => {
    const w = mc("walk2", t, TS, 0.8, 0.9); const tx = lerp(-160, 560, mix(t, 0, 6.2, "linear"));
    ...
    <AbsoluteFill style={{ background: "#000" }}>
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ position: "absolute", left: 0, top: 0 }}>
        ...
        <g transform={`translate(${W / 2} ${H / 2}) scale(${zoom}) translate(${-W / 2 - camX} ${-H / 2 - camY})`}>
          ...
          {f.tort && <Character id="june" st={f.tort} ground={g} />}
          {f.hare && <Character id="rusty" st={f.hare} ground={g} />}
          {f.fx}
        </g>
      </svg>
