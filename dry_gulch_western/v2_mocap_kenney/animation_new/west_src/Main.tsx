...
import { Character, CharState, Defs, Pose, addPose, blinkAt, ease, followThrough, gait, lerp, lerpPose, mix, noise, seq, mocapPose } from "./Rig";
// CMU mocap gaits (see mocap_clips.ts). mc() returns {pose,bob} shaped like gait(); speed picked so foot travel ≈ screen travel.
const LEG_H = 146.8, LEG_T = 749.8;
const mc = (clip: string, t: number, legLen: number, scale: number, speed = 1, amp = 1) => { const m = mocapPose(clip, t, { legLen, speed, amp }); return { pose: m.pose, bob: m.hipY * scale, lean: 0, travel: m.travel * scale }; };
...
export type Frame = { hare?: CharState; tort?: CharState; camX?: number; camY?: number; zoom?: number; fx?: React.ReactNode; fxBack?: React.ReactNode; cans?: boolean[] };
export type Sc = { id: string; start: number; end: number; bg: BgKind; solve: (t: number) => Frame };
...
export const SCENES: Sc[] = [
  ...
  { id: "sc1", start: 0, end: 15.0, bg: "street", solve: (t) => {
    ...
    const g = mc("walk", t, LEG_H, HS, 0.75);
  ...
  { id: "sc2", start: 15.0, end: 21.3, bg: "street", solve: (t) => {
    const w = mc("walk2", t, LEG_T, TS, 0.32, 0.9); const tx = lerp(-160, 560, mix(t, 0, 6.2, "linear"));
  ...
  { id: "sc6", start: 44.1, end: 52.2, bg: "porch", solve: (t) => {
    const g = mc("walk", t, LEG_H, HS, 1.0);
  ...
  { id: "sc8", start: 58.6, end: 66.1, bg: "fence", solve: (t) => {
    ...
    const g = mc("run", t - 1.6, LEG_H, HS, 0.85);
...
export type OvKey = { t: number; ease?: string; pose?: Pose; x?: number; y?: number; hipsY?: number; face?: string };
export type Overrides = Record<string, Record<string, OvKey[]>>; // sceneId -> charId -> keys (t = scene-local seconds)
export const applyOverrides = (f: Frame, sc: Sc, lt: number, ov?: Overrides): Frame => {
  const so = ov?.[sc.id]; if (!so) return f;
  const out: any = { ...f };
  for (const cid of ["hare", "tort"] as const) {
    const keys = (so[cid] || []).slice().sort((a, b) => a.t - b.t); const c = out[cid]; if (!keys.length || !c) continue;
    let a = keys[0], b = keys[0]; for (const k of keys) { if (k.t <= lt) a = k; if (k.t >= lt) { b = k; break; } b = k; }
    const p = b.t > a.t ? (ease as any)[b.ease || "inOut"](Math.max(0, Math.min(1, (lt - a.t) / (b.t - a.t)))) : 1;
    const pa = a.pose || {}, pb = b.pose || {}; const delta: Pose = {};
    new Set([...Object.keys(pa), ...Object.keys(pb)]).forEach((k) => { delta[k] = { rot: lerp(pa[k]?.rot ?? 0, pb[k]?.rot ?? 0, p), y: lerp(pa[k]?.y ?? 0, pb[k]?.y ?? 0, p) }; });
    const pose = addPose(c.pose, delta);
    out[cid] = { ...c, pose, x: c.x + lerp(a.x ?? 0, b.x ?? 0, p), y: (c.y ?? 0) + lerp(a.y ?? 0, b.y ?? 0, p), face: (p < 0.5 ? a.face : b.face) || c.face };
  }
  return out;
};
export type Props = { words: Word[]; audio: string; clip?: { start: number; end: number }; overrides?: Overrides };
export const Main: React.FC<Props> = ({ words, audio, clip, overrides }) => {
  const frame = useCurrentFrame(); const { fps } = useVideoConfig();
  const t = frame / fps + (clip?.start ?? 0);
  const sc = SCENES.find((s) => t >= s.start && t < s.end) ?? SCENES[SCENES.length - 1];
  const lt = t - sc.start; const f = applyOverrides(sc.solve(lt), sc, lt, overrides); const g = GROUND[sc.bg];
